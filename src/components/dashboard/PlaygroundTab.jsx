...
import { Send, Trash2, ExternalLink, Settings, ArrowRight } from 'lucide-react';
...
export default function PlaygroundTab({ businessId, bot, onNavigate }) {
    ...
    const handleSend = async (e) => {
        ...
        try {
            const res = await axios.post(
                ...
                { query: userMsg, business_id: businessId, visitor_id: 'dashboard-playground-user' },
            ...
            const errMsg = err.response?.data?.error || err.response?.data?.message || err.message || 'Connection error while reaching the server.';
    ...
    return (
        <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <div className="playground-mobile-bar">
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1, minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: '0.82rem', fontWeight: 600, color: '#7a5c00' }}>Testing Mode</span>
                    <span style={{ fontSize: '0.82rem', color: '#a07020', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>— {bot?.bot_name || 'BeeBot'}</span>
                </div>
                <button
                    onClick={() => onNavigate?.('settings')}
                    style={{ display: 'flex', alignItems: 'center', gap: '5px', background: '#000', color: '#FDD017', border: 'none', padding: '7px 12px', borderRadius: '7px', fontFamily: 'inherit', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}
                >
                    <Settings size={12} /> Edit Settings <ArrowRight size={11} />
                </button>
            </div>

            {/* ── Main grid: sidebar (desktop) + chat ───────────────────────── */}
            <div className="playground-layout">

                {/* ── Left Panel: Settings Sidebar (desktop only) ───────────── */}
                ...
                <div className="playground-chat">
                    ...
                    <div style={{ background: '#f0f0f0', padding: '8px 12px', borderBottom: '1px solid #ddd', display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                        ...
                        <div style={{ flex: 1, background: '#e0e0e0', borderRadius: '6px', padding: '3px 10px', fontSize: '0.72rem', color: '#888', fontFamily: 'var(--font-mono)', textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            yourwebsite.com
                        </div>
                        <div style={{ display: 'flex', gap: '4px', flexShrink: 0 }}>
                            <button
                                onClick={resetConversation}
                                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#888', display: 'flex', alignItems: 'center', gap: '3px', fontSize: '0.72rem', padding: '3px 6px', borderRadius: '5px', fontFamily: 'inherit' }}
                                title="Reset conversation"
                            >
                                <Trash2 size={12} /> <span style={{ display: 'none' }}>Reset</span>
                                <span className="hide-on-mobile" style={{ fontSize: '0.72rem' }}>Reset</span>
                            </button>
                            <a
                                href="/playground-preview"
                                target="_blank"
                                rel="noopener noreferrer"
                                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#888', display: 'flex', alignItems: 'center', gap: '3px', fontSize: '0.72rem', padding: '3px 6px', borderRadius: '5px', textDecoration: 'none' }}
                    ...
                    {/* Chat header */}
                    <div style={{ padding: '12px 14px', background: primaryColor, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                <img src="/bee-chat.png" alt="" style={{ width: '32px', height: '32px', borderRadius: '50%', objectFit: 'cover' }} onError={e => e.target.style.display = 'none'} />
                            </div>
                            <div>
                                <div style={{ fontWeight: 600, fontSize: '0.88rem', color: '#fff', lineHeight: 1.2 }}>{bot?.bot_name || 'BeeBot'}</div>
                                <div style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.75)', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '1px' }}>
                                    <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#4ade80', display: 'inline-block' }} /> Online
                                </div>
                            </div>
                        </div>
                        <span style={{ fontSize: '0.68rem', background: 'rgba(0,0,0,0.2)', color: 'rgba(255,255,255,0.85)', padding: '3px 8px', borderRadius: '20px', fontWeight: 600, whiteSpace: 'nowrap' }}>
                            Testing Mode
                        </span>
                    </div>

                    {/* Messages */}
                    <div style={{ flex: 1, overflowY: 'auto', padding: '14px 12px', background: '#f7f8fa', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {messages.map((msg, idx) => (
                            <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                                <div style={{
                                    maxWidth: '82%', padding: '9px 13px', fontSize: '0.87rem', lineHeight: 1.55, wordBreak: 'break-word',
                    ...
                    <div style={{ padding: '10px 12px', borderTop: '1px solid #efefef', background: '#fff', flexShrink: 0 }}>
                        <form onSubmit={handleSend} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                            <input
                                ...
                                disabled={isLoading}
                                style={{ flex: 1, padding: '9px 14px', background: '#f9fafb', border: '1.5px solid #e2e8f0', borderRadius: '20px', fontSize: '0.85rem', outline: 'none', fontFamily: 'inherit', color: '#1a1a1a', transition: 'border-color 0.15s', minWidth: 0 }}
                        ...
                        <div style={{ textAlign: 'center', marginTop: '5px', fontSize: '0.62rem', color: '#bbb' }}>Powered by BeeBot</div>
                    </div>
                </div>
            </div>
        </div>
